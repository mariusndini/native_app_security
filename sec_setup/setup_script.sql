...
AS $$
  ...
  BEGIN
    ...
    res := (EXECUTE IMMEDIATE 'CALL CORE.MAKE_BILLING_EVENT(
              select count(distinct asset_id) from reference(\'holdings_table\') )' ); 

    -- SELECT SYSTEM$CREATE_BILLING_EVENT( 'BILLING_EVENT','',1689615708000, 1689615708000 , $X - CHARGE(s),'','');
